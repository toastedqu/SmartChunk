...
def get_qa_cos_sim(dataset_or_queries, answers):
    if isinstance(dataset_or_queries, str):
        queries_embeddings = get_query_embeddings(dataset_or_queries)
    else:
        queries_embeddings = ENCODER.encode_queries(dataset_or_queries)
...
def get_query_embeddings(dataset):
    queries = json.load(open(f'datasets/{dataset}/queries.json', 'r'))
    sampled_queries = json.load(open(f'datasets/{dataset}/sampled_queries.json', 'r'))
    queries_embeddings = np.load(f'embeddings/{dataset}/queries.npy')
    all_query_keys = list(sorted(list(queries.keys())))
    sampled_query_keys = list(sorted(list(sampled_queries.keys())))
    sampled_query_idx = [all_query_keys.index(query_id) for query_id in sampled_query_keys]
    return queries_embeddings[sampled_query_idx]
...
def evaluate_generation(cfg):
    ...
    qa_sim = get_qa_cos_sim(dataset, l_generated_answers)
...
if __name__ == "__main__":
    ...
    with open(f"results/results_generation_new.pkl", "wb") as f:
