from cluster import linkage_info, linkage_k_split, linkage_k_preserve, linkage_expand_cluster
...
def __sim_pos_dist__(A: np.ndarray, B: np.ndarray) -> float:
    A_pos = int(A[0])
    B_pos = int(B[0])
...
# k-split mode: Use a cluster as a chunk, by clustering sentences in the document, then split the document into k chunks
# k-preserve mode: Use a cluster as a chunk, but guarantee every chunk contains at least k sentences
# This chunker does not guarantee but consider the continuity of sentences, but it only preserves the order
def cluster_chunker(doc: str, doc_id: str, k=4, continuity=True, mode: typing.Literal["k-split", "k-preserve"] = "k-split"):
    texts = []
    ids = []
    sents = [sent.text for sent in nlp(doc).sents]
    if len(sents) == 1:  # no need to cluster
    ...
    if continuity:
        ...
        linkage_matrix = linkage(candidate_emb, method="single", metric=__sim_pos_dist__)
    else:
    ...
    if mode == "k-split":
        # indices of sentences in the split
        k_split_idx = linkage_k_split(linkage_matrix, len(sents), linkage_size, k)
    elif mode == "k-preserve":
        k_split_idx = linkage_k_preserve(linkage_matrix, len(sents), linkage_size, k)
    else:
        raise ValueError("mode must be either k-split or k-preserve")
    k_split_idx = sorted(list(k_split_idx))
    for cluster_id in k_split_idx:
        leaf_ids = linkage_expand_cluster(cluster_id, linkage_matrix, len(sents))
...
def sent_cont_chunker(doc: str, doc_id: str, k=1):
    texts = []
    ids = []
    sents = [sent.text for sent in nlp(doc).sents]
    buffer = ""
    for sid, sent in enumerate(sents):
        if sid % k == k - 1:
            ...
            ids.append("{}s{}".format(doc_id, int(sid)))  # s = split
            buffer = ""
        else:
...
def arbitrary_chunker(doc: str, doc_id: str, k=100):
    texts = []
    ids = []
    words = doc.split(" ")
    buffer = ""
    for wid, word in enumerate(words):
        if wid % k == k - 1:
            ...
            ids.append("{}s{}".format(doc_id, int(wid)))  # s = split
            buffer = ""
        else:
